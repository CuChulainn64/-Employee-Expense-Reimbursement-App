...
public class UserDAOImpl implements UserDAO{
	...
	public UserRole getUserRoleById(int id, Session ses) {
		...
		UserRole role = (UserRole) ses.createNativeQuery("SELECT * FROM UserRole WHERE userRole_id=" + id + "", UserRole.class).getSingleResult();
	...
	public User selectById(int id) {
		...
		Session ses = HibernateUtil.getSession();
		User user = (User) ses.createNativeQuery("SELECT * FROM User WHERE user_id = " + id + "", User.class).getSingleResult();
	...
	public User selectByName(String name) {
		...
		Session ses = HibernateUtil.getSession();
		User user = (User) ses.createNativeQuery("SELECT * FROM User WHERE user_firstname = '" + name + "'", User.class).getSingleResult();
	...
	public boolean update(User user) {
		...
		//ses.update(user);
		String sql = String.format("update User set user_username='%s', user_password='%s', user_firstname='%s', user_lastname='%s' where user_id=%d", user.getUsername(), user.getPassword(), user.getFirstName(), user.getLastName(), user.getId());
